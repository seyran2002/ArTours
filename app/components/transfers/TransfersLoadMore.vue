...
<template>
  <div v-if="totalFiltered > 0" class="flex flex-col items-center gap-4 pt-10 sm:pt-14">
    ...
    <p class="text-xs font-semibold text-zinc-500 tracking-wide">
      {{ $t('transfers.showing') }} 
      <span class="text-zinc-700">{{ Math.min(visibleCount, totalFiltered) }}</span> 
      {{ $t('transfers.of') }} 
      <span class="text-zinc-700">{{ totalFiltered }}</span> 
      {{ $t('transfers.transfersCount') }}
    </p>
    ...
    >
      <span class="mr-2">{{ $t('transfers.loadMore') }}</span>
      <BaseIcon
        name="chevron-down"
        size="xs"
