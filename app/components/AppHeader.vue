...
<template>
  ...
  <header class="absolute md:fixed top-0 z-50 w-full transition-all duration-500">
    ...
    <div class="relative max-w-[1440px] mx-auto px-3.5 sm:px-5 lg:px-8">
      <div class="flex items-center justify-between h-16 sm:h-[72px] md:h-20">
        ...
        >
          <img
            ...
            fetchpriority="high"
            decoding="async"
            class="h-10 sm:h-12 md:h-12 lg:h-16 w-auto object-contain transition-all duration-500 ease-out group-hover:scale-[1.02] group-active:scale-[0.98]"
          />
        </NuxtLink>
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          ...
          <NuxtLink
            :to="localePath('/tours')"
            class="flex items-center gap-1.5 lg:gap-2 bg-brand-gradient text-white pl-3 sm:pl-3.5 lg:pl-5 pr-2.5 sm:pr-3 lg:pr-4 py-1.5 sm:py-2 lg:py-2.5 rounded-full text-[11px] sm:text-xs lg:text-[13px] font-semibold tracking-[-0.01em] shadow-brand transition-all duration-400 ease-out hover:scale-[1.03] hover:shadow-brand-hover active:scale-[0.98] outline-none select-none group focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2 whitespace-nowrap shrink-0"
            aria-label="Browse tours and book now"
          >
            <span>{{ t('header.bookNow') }}</span>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-300 group-hover:translate-x-0.5">
  ...
  <!-- ═══════════════════════════════════════════════════════════ -->
  <div class="md:hidden fixed bottom-0 left-0 right-0 z-50 px-2 sm:px-4 pb-[max(env(safe-area-inset-bottom,8px),8px)] pt-1.5 pointer-events-none">
    <nav
      :class="[
        'pointer-events-auto mx-auto w-full max-w-[460px] rounded-2xl px-1.5 py-1 sm:py-1.5 flex items-center justify-between transition-all duration-500',
        isTransparent 
    ...
    >
      ...
      >
        ...
        >
          <!-- Icon -->
          <div class="relative flex items-center justify-center h-5 mb-0.5">
            <BaseIcon
              :name="link.icon"
              :size="19"
              :custom-class="[
